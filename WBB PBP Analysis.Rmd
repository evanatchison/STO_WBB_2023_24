...
pbp2 <- read_csv("PBP_Data_Through_2_16.csv")
```
...
shooting_by_quarter2 <- shooting_by_quarter %>%
  filter(checkname52 != "elstad,maddie", checkname52 != "henson,megan", checkname52 != "scott,camryn")
...
quarterly_shooting <- function(player){
  shooting_by_quarter %>%
    ...
    geom_point() +
    geom_smooth() +
    labs(title = str_c(str_to_upper({{player}}), " Shooting by Quarter"),
         y = "shooting percentage") +
    theme_classic()
}
...
quarterly_shooting("howdeshell")
quarterly_shooting("kelly")
quarterly_shooting("kormann")
quarterly_shooting("lewis")
quarterly_shooting("olmen")
quarterly_shooting("tangen")
quarterly_shooting("yagodinski")

#strong first quarter players: botten, fix, hegland, kelly, olmen
#strong second quarter players: olmen
#strong third quarter players: kormann, lewis, tangen, yagodinski
#strong fourth quarter players: fix, kelly, kormann, tangen
...
rebounding <- function(player){
  lineups %>%
    ...
    left_join(pbp2) %>%
    mutate(checkname52 = tolower(checkname52)) %>%
    summarise(rebound_rate = 100 * (sum(action == "REBOUND" &
                                   str_detect(checkname52, {{player}}))
                                   * total_mins)/
